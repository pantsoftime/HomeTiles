...
bool init_panel_framebuffer() {
  ...
  const esp_err_t err = esp_lcd_dpi_panel_get_frame_buffer(g_panel,
                                                           ...
                                                           &fb0);
  if (err != ESP_OK || !fb0) {
    Serial.printf("[Device/WaveshareTouchLCD7] Panel framebuffer unavailable err=%d fb=%p\n",
  ...
  reset_dirty_rect();
  Serial.printf("[Device/WaveshareTouchLCD7] Panel framebuffer OK fb=%p\n", fb0);
  return true;
}
      ...
      if (g_ppa_async_ready && g_ppa_done) {
        ...
        if (err == ESP_OK) {
          if (xSemaphoreTake(g_ppa_done, pdMS_TO_TICKS(kPpaRotateTimeoutMs)) == pdTRUE) {
            ...
            Serial.printf("[Device/WaveshareTouchLCD7] PPA rotate timeout x=%ld y=%ld w=%ld h=%ld -> CPU cooldown\n",
                          static_cast<long>(x), static_cast<long>(y),
                          static_cast<long>(w), static_cast<long>(h));
            note_ppa_fault();
          }
