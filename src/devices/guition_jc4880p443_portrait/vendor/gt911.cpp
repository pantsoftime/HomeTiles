...
// Function to initialize the GT911 touch controller. Without RST/INT the
// address depends on strapping, so both addresses are probed at 400/100 kHz.
esp_lcd_touch_handle_t touch_gt911_init(DEV_I2C_Port port)
{
    static const uint32_t kProbeSpeedsHz[] = {400000, 100000};
    static const uint8_t kProbeAddrs[] = {
    ...
    for (uint8_t s = 0; s < sizeof(kProbeSpeedsHz) / sizeof(kProbeSpeedsHz[0]); ++s) {
        for (uint8_t a = 0; a < sizeof(kProbeAddrs) / sizeof(kProbeAddrs[0]); ++a) {
            const uint8_t touch_addr = kProbeAddrs[a];
            if (i2c_master_probe(port.bus, touch_addr, 100) != ESP_OK) {
                continue;
            }
            ESP_LOGI(TAG, "Trying touch address 0x%02X at %lu Hz",
                     touch_addr, static_cast<unsigned long>(kProbeSpeedsHz[s]));

            esp_lcd_panel_io_handle_t tp_io_handle = NULL;
            esp_lcd_panel_io_i2c_config_t tp_io_config = ESP_LCD_TOUCH_IO_I2C_GT911_CONFIG();
            tp_io_config.dev_addr = touch_addr;
            tp_io_config.scl_speed_hz = kProbeSpeedsHz[s];
            ...
            if (err == ESP_OK && tp_handle) {
                ESP_LOGI(TAG, "GT911 touch ready at 0x%02X (%lu Hz)",
                         touch_addr, static_cast<unsigned long>(kProbeSpeedsHz[s]));
                return tp_handle;
            }
    ...
    ESP_LOGE(TAG, "GT911 probe failed on 0x%02X and 0x%02X at 400/100 kHz",
             ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS,
             ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS_BACKUP);
    return NULL;
}
...
static esp_err_t touch_gt911_read_cfg(esp_lcd_touch_handle_t tp)
{
    ...
    Serial.printf("[Device/GuitionJC4880P443Portrait] TouchPad_ID:%02X%02X%02X cfg=%u\n",
