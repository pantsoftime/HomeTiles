...
#include "src/devices/waveshare_touch_lcd_10_1/vendor/i2c.h"
#include "src/devices/waveshare_touch_lcd_10_1/vendor/jd9365/esp_lcd_jd9365_10_1.h"
#include "src/devices/waveshare_touch_lcd_10_1/vendor/panel_init_cmds.h"
...
uint8_t g_rotation = DeviceWaveshareTouchLCD10::kProfile.rotation_default;
...
void log_step(const char* message) {
  Serial.print("[Device/WaveshareTouchLCD10] ");
...
void reset_ppa_client() {
  ...
  if (old) {
    ...
    if (unregister_err != ESP_OK) {
      ...
      Serial.printf("[Device/WaveshareTouchLCD10] PPA client busy err=%d, retry reset in %lu ms\n",
  ...
  if (reg_err != ESP_OK || !fresh) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.printf("[Device/WaveshareTouchLCD10] PPA reset failed err=%d (int free=%u KB, largest=%u KB), retry in %lu ms\n",
  ...
  if (!g_ppa_async_ready) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.println("[Device/WaveshareTouchLCD10] PPA timeout callback unavailable, retrying client reset");
    return;
  }
  g_ppa_reinit_at_ms = 0;
  g_ppa_reset_pending = false;
  Serial.println("[Device/WaveshareTouchLCD10] PPA client reset after fault");
}
...
void wait_transfer_done(size_t pixels) {
  ...
  if (xSemaphoreTake(g_transfer_done, timeout) != pdTRUE) {
    Serial.printf("[Device/WaveshareTouchLCD10] draw transfer timeout (%u px)\n",
...
void wait_refresh_done() {
  ...
  if (xSemaphoreTake(g_refresh_done, pdMS_TO_TICKS(50)) != pdTRUE) {
    Serial.println("[Device/WaveshareTouchLCD10] refresh timeout");
  }
}
...
bool ensure_rotate_buffer(size_t pixels) {
  ...
  if (!g_rotate_buf) {
    Serial.printf("[Device/WaveshareTouchLCD10] Rotate buffer allocation failed (%u bytes)\n",
...
bool draw_physical(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/WaveshareTouchLCD10] Reject out-of-range draw x=%ld y=%ld w=%ld h=%ld\n",
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] draw_bitmap failed: %d\n", static_cast<int>(err));
    return false;
  }
...
bool write_physical_to_panel(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  const int32_t panel_h = display_cfg.height;
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/WaveshareTouchLCD10] Reject out-of-range framebuffer draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
...
bool draw_landscape_area(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > logical_w || (y + h) > logical_h) {
    Serial.printf("[Device/WaveshareTouchLCD10] Reject out-of-range landscape draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
...
bool init_backlight() {
  ...
  if (ledc_timer_config(&timer_cfg) != ESP_OK) {
    Serial.println("[Device/WaveshareTouchLCD10] Backlight timer init failed");
    return false;
  }
  ...
  if (ledc_channel_config(&ch_cfg) != ESP_OK) {
    Serial.println("[Device/WaveshareTouchLCD10] Backlight channel init failed");
    return false;
  }
...
bool init_i2c() {
  ...
  g_i2c = DEV_I2C_Init();
  if (!g_i2c.bus) {
    Serial.println("[Device/WaveshareTouchLCD10] I2C init failed");
    return false;
  }
  g_i2c_ready = true;
  return true;
...
bool init_mipi_phy_power() {
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] MIPI PHY LDO init failed err=%d\n",
                  static_cast<int>(err));
    return false;
  }
...
bool init_display() {
  ...
  if (!g_transfer_done || !g_refresh_done) {
    Serial.println("[Device/WaveshareTouchLCD10] Transfer semaphore allocation failed");
    return false;
  }
  ...
  // ESP32-P4 v3.x builds ("v3.00 or newer") accept XTAL/APLL/CPLL/SPLL/MPLL
  // only; PLL_F20M aborts in the low-level MIPI DSI clock-source switch.
  bus_cfg.phy_clk_src = MIPI_DSI_PHY_PLLREF_CLK_SRC_DEFAULT;
#else
  // Pre-v3 builds ("Before v3.00") accept PLL_F20M/RC_FAST/PLL_F25M only;
  // XTAL/default aborts in the low-level MIPI DSI clock-source switch.
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] DSI bus init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] DSI DBI IO init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  vendor_cfg.init_cmds = kWaveshareJd9365Init10_1;
  vendor_cfg.init_cmds_size =
      static_cast<uint16_t>(kWaveshareJd9365Init10_1Count);
  ...
  err = esp_lcd_new_panel_jd9365_10_1(g_panel_io, &panel_cfg, &g_panel);
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] Panel create failed err=%d\n",
                  static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] DPI callback register failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] Panel reset failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] Panel init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] Panel display on failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (ppa_err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD10] PPA client register failed err=%d, falling back to CPU rotate\n",
    ...
    if (g_ppa_done) {
      ...
      if (ppa_client_register_event_callbacks(g_ppa_handle, &ppa_cbs) == ESP_OK) {
        ...
        Serial.println("[Device/WaveshareTouchLCD10] PPA event cb register failed, retrying safely");
      }
    }
    if (!g_ppa_async_ready) {
      g_ppa_reset_pending = true;
...
bool init_touch() {
  ...
  for (uint8_t attempt = 1; attempt <= 3; ++attempt) {
    ...
    Serial.printf("[Device/WaveshareTouchLCD10] Touch init attempt %u failed\n",
                  static_cast<unsigned>(attempt));
    delay(150);
  }

  if (!g_touch) {
    Serial.println("[Device/WaveshareTouchLCD10] Touch init failed");
    return false;
  }
...
bool DeviceWaveshareTouchLCD10::init() {
  Serial.println("[Device/WaveshareTouchLCD10] Initialising board...");
  ...
  Serial.println("[Device/WaveshareTouchLCD10] Display OK");

  if (!init_touch()) {
    Serial.println("[Device/WaveshareTouchLCD10] Touch init failed, continuing without touch");
  } else {
    Serial.println("[Device/WaveshareTouchLCD10] Touch OK");
  }
  ...
  displayFillScreen(0x0000);
  apply_backlight(0);
  Serial.println("[Device/WaveshareTouchLCD10] Init complete");
  return true;
}

void DeviceWaveshareTouchLCD10::update() {
}

void DeviceWaveshareTouchLCD10::pausePpaFor(uint32_t duration_ms) {
  pause_ppa_for(duration_ms);
}

bool DeviceWaveshareTouchLCD10::ppaCooldownActive() {
  return ppa_cooldown_active();
}

void DeviceWaveshareTouchLCD10::displayPushPixels(int32_t x, int32_t y, int32_t w, int32_t h,
                                                 const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}

void DeviceWaveshareTouchLCD10::displayPushPixelsDMA(int32_t x, int32_t y, int32_t w, int32_t h,
                                                    const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}
...
void DeviceWaveshareTouchLCD10::displayCommit() {
...
void DeviceWaveshareTouchLCD10::displayFillScreen(uint16_t color) {
  if (!g_panel) {
    return;
  }
...
void DeviceWaveshareTouchLCD10::displaySetRotation(uint8_t rotation) {
  g_rotation = rotation & 0x03;
}

void DeviceWaveshareTouchLCD10::setBrightness(uint8_t value) {
  g_brightness = value;
  apply_backlight(value);
}

uint8_t DeviceWaveshareTouchLCD10::getBrightness() {
  return g_brightness;
}

bool DeviceWaveshareTouchLCD10::getTouch(int16_t& x, int16_t& y) {
  if (!g_touch) {
    return false;
  }
...
void DeviceWaveshareTouchLCD10::displaySleep() {
  if (g_panel) {
...
void DeviceWaveshareTouchLCD10::displayWake() {
  if (g_panel) {
...
void DeviceWaveshareTouchLCD10::displayWakeDark() {
  if (g_panel) {
    esp_lcd_panel_disp_on_off(g_panel, true);
  }
  apply_backlight(0);
...
void DeviceWaveshareTouchLCD10::displayPowerSaveOn() {
  displaySleep();
}

void DeviceWaveshareTouchLCD10::displayPowerSaveOff() {
  displayWake();
}

void DeviceWaveshareTouchLCD10::displayWaitDisplay() {
}

void DeviceWaveshareTouchLCD10::prepareForRestart() {
  if (g_panel) {
    displayFillScreen(0x0000);
    esp_lcd_panel_disp_on_off(g_panel, false);
  }
...
bool DeviceWaveshareTouchLCD10::initSDCard() {
  ...
  if (!WaveshareSDMMC.begin("/sdcard", false, SDMMC_FREQ_DEFAULT, 5)) {
    ...
    Serial.println("[Device/WaveshareTouchLCD10] SD card mount failed");
    return false;
  }
  ...
  if (card_type == CARD_NONE) {
    g_sd_available = false;
    Serial.println("[Device/WaveshareTouchLCD10] SD card absent after mount");
    WaveshareSDMMC.end();
    return false;
  }

  g_sd_available = true;
  Serial.printf("[Device/WaveshareTouchLCD10] SD card OK, type=%u, size=%llu MB\n",
                static_cast<unsigned>(card_type),
                static_cast<unsigned long long>(WaveshareSDMMC.cardSize() / (1024ULL * 1024ULL)));
  return true;
}

bool DeviceWaveshareTouchLCD10::storageReady() {
  return g_littlefs_ready;
}

fs::FS& DeviceWaveshareTouchLCD10::storageFS() {
  return LittleFS;
}

bool DeviceWaveshareTouchLCD10::sdReady() {
  return initSDCard();
}

fs::FS& DeviceWaveshareTouchLCD10::sdFS() {
  return WaveshareSDMMC;
}

bool DeviceWaveshareTouchLCD10::suspendSDCardForNetworkTransition() {
  ...
  Serial.println("[Device/WaveshareTouchLCD10] SD card suspended for network transition");
  return true;
}

bool DeviceWaveshareTouchLCD10::resumeSDCardAfterNetworkTransition() {
  g_sd_available = false;
  g_sd_init_attempted = false;
  const bool mounted = initSDCard();
  Serial.printf("[Device/WaveshareTouchLCD10] SD card remount after network transition: %s\n",
                mounted ? "OK" : "failed");
  return mounted;
}

bool DeviceWaveshareTouchLCD10::initLittleFS() {
  if (g_littlefs_ready) {
    return true;
  }
  if (!LittleFS.begin(true, "/littlefs", 10, "spiffs")) {
    Serial.println("[Device/WaveshareTouchLCD10] LittleFS mount failed");
    return false;
  }
  g_littlefs_ready = true;
  Serial.printf("[Device/WaveshareTouchLCD10] LittleFS ready, total=%u, used=%u\n",
...
void DeviceWaveshareTouchLCD10::migrateStorageFromSD() {
  if (!initLittleFS()) {
    return;
  }
