...
#include "src/devices/guition_jc1060p470c/vendor/displays_config.h"
#include "src/devices/guition_jc1060p470c/vendor/esp_lcd_jd9165.h"
#include "src/devices/guition_jc1060p470c/vendor/gt911.h"
#include "src/devices/guition_jc1060p470c/vendor/guition_sdmmc.h"
#include "src/devices/guition_jc1060p470c/vendor/i2c.h"
...
void log_step(const char* message) {
  Serial.print("[Device/Guition JC1060P470C] ");
...
void reset_ppa_client() {
  ...
  if (old) {
    ...
    if (unregister_err != ESP_OK) {
      ...
      Serial.printf("[Device/Guition JC1060P470C] PPA client busy err=%d, retry reset in %lu ms\n",
  ...
  if (reg_err != ESP_OK || !fresh) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.printf("[Device/Guition JC1060P470C] PPA reset failed err=%d (int free=%u KB, largest=%u KB), retry in %lu ms\n",
  ...
  if (!g_ppa_async_ready) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.println("[Device/Guition JC1060P470C] PPA timeout callback unavailable, retrying client reset");
    return;
  }
  g_ppa_reinit_at_ms = 0;
  g_ppa_reset_pending = false;
  Serial.println("[Device/Guition JC1060P470C] PPA client reset after fault");
}
...
void wait_transfer_done(size_t pixels) {
  ...
  if (xSemaphoreTake(g_transfer_done, timeout) != pdTRUE) {
    Serial.printf("[Device/Guition JC1060P470C] draw transfer timeout (%u px)\n",
...
void wait_refresh_done() {
  ...
  if (xSemaphoreTake(g_refresh_done, pdMS_TO_TICKS(50)) != pdTRUE) {
    Serial.println("[Device/Guition JC1060P470C] refresh timeout");
  }
}
...
bool ensure_rotate_buffer(size_t pixels) {
  ...
  if (!g_rotate_buf) {
    Serial.printf("[Device/Guition JC1060P470C] Rotate buffer allocation failed (%u bytes)\n",
...
bool draw_physical(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/Guition JC1060P470C] Reject out-of-range draw x=%ld y=%ld w=%ld h=%ld\n",
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] draw_bitmap failed: %d\n", static_cast<int>(err));
    return false;
  }
...
bool init_panel_framebuffer() {
  ...
  if (err != ESP_OK || !fb0 || !fb1) {
    Serial.printf("[Device/Guition JC1060P470C] Panel framebuffers unavailable err=%d fb0=%p fb1=%p\n",
  ...
  if (!g_panel_fb_ready) {
    Serial.println(
        "[Device/Guition JC1060P470C] Shared camera presenter init failed");
    return false;
  }
  reset_dirty_rect();
  Serial.printf("[Device/Guition JC1060P470C] Panel framebuffers OK fb0=%p fb1=%p\n",
                fb0, fb1);
  return true;
}
...
bool write_physical_to_panel(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  const int32_t panel_h = display_cfg.height;
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/Guition JC1060P470C] Reject out-of-range framebuffer draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
...
bool draw_landscape_area(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > logical_w || (y + h) > logical_h) {
    Serial.printf("[Device/Guition JC1060P470C] Reject out-of-range landscape draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
    ...
    if (fb) {
        ...
        if (err == ESP_OK) {
          if (xSemaphoreTake(g_ppa_done,
            ...
            p4_dsi_camera_presenter::restartAfterPpaTimeout(
                "GuitionJC1060P470C", "LVGL PPA rotation", x, y, w, h, w,
                g_rotation, kPpaRotateTimeoutMs + kPpaPreviewGraceMs);
          }
        } else {
          Serial.printf("[Device/Guition JC1060P470C] PPA rotate submit failed err=%d -> CPU cooldown\n",
                        static_cast<int>(err));
          note_ppa_fault();
        }
      }
      ...
      if (ppa_ok) {
        if (!g_camera_presenter.noteUiWrite(
          ...
          Serial.println(
              "[Device/Guition JC1060P470C] UI framebuffer cache sync failed");
          return false;
        }
...
bool init_backlight() {
  ...
  if (ledc_timer_config(&timer_cfg) != ESP_OK) {
    Serial.println("[Device/Guition JC1060P470C] Backlight timer init failed");
    return false;
  }
  ...
  if (ledc_channel_config(&ch_cfg) != ESP_OK) {
    Serial.println("[Device/Guition JC1060P470C] Backlight channel init failed");
    return false;
  }
...
bool init_i2c() {
  ...
  g_i2c = DEV_I2C_Init();
  if (!g_i2c.bus) {
    Serial.println("[Device/Guition JC1060P470C] I2C init failed");
    return false;
  }
  g_i2c_ready = true;
  return true;
...
bool init_mipi_phy_power() {
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] MIPI PHY LDO init failed err=%d\n",
                  static_cast<int>(err));
    return false;
  }
...
bool init_display() {
  ...
  if (!g_transfer_done || !g_refresh_done) {
    Serial.println("[Device/Guition JC1060P470C] Transfer semaphore allocation failed");
    return false;
  }
  ...
  Serial.printf("[Device/Guition JC1060P470C] DSI bus init start lane=%u Mbps\n",
                static_cast<unsigned>(display_cfg.lane_bit_rate));
  Serial.flush();
  esp_err_t err = esp_lcd_new_dsi_bus(&bus_cfg, &g_dsi_bus);
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] DSI bus init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] DSI DBI IO init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] JD9165 panel create failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] DPI callback register failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] Panel reset failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] Panel init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] Panel display on failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (ppa_err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] PPA client register failed err=%d, falling back to CPU rotate\n",
    ...
    if (g_ppa_done) {
      ...
      if (ppa_client_register_event_callbacks(g_ppa_handle, &ppa_cbs) == ESP_OK) {
        ...
        Serial.println("[Device/Guition JC1060P470C] PPA event cb register failed, retrying safely");
      }
    }
    if (!g_ppa_async_ready) {
      g_ppa_reset_pending = true;
...
bool init_touch() {
  ...
  for (uint8_t attempt = 1; attempt <= 3; ++attempt) {
    ...
    Serial.printf("[Device/Guition JC1060P470C] Touch init attempt %u failed\n",
                  static_cast<unsigned>(attempt));
    delay(150);
  }

  if (!g_touch) {
    Serial.println("[Device/Guition JC1060P470C] Touch init failed");
    return false;
  }
...
bool DeviceGuitionJC1060P470C::init() {
  Serial.println("[Device/Guition JC1060P470C] Initialising board...");
  ...
  Serial.println("[Device/Guition JC1060P470C] Display OK");

  if (!init_touch()) {
    Serial.println("[Device/Guition JC1060P470C] Touch init failed, continuing without touch");
  } else {
    Serial.println("[Device/Guition JC1060P470C] Touch OK");
  }
  ...
  displayFillScreen(0x0000);
  apply_backlight(0);
  Serial.println("[Device/Guition JC1060P470C] Init complete");
  return true;
}

void DeviceGuitionJC1060P470C::update() {
}

void DeviceGuitionJC1060P470C::pausePpaFor(uint32_t duration_ms) {
  pause_ppa_for(duration_ms);
}

bool DeviceGuitionJC1060P470C::ppaCooldownActive() {
  return ppa_cooldown_active();
}

void DeviceGuitionJC1060P470C::displayPushPixels(int32_t x, int32_t y, int32_t w, int32_t h,
                                                 const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}

void DeviceGuitionJC1060P470C::displayPushPixelsDMA(int32_t x, int32_t y, int32_t w, int32_t h,
                                                    const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}

bool DeviceGuitionJC1060P470C::displayTryFullFramePreview(
...
void DeviceGuitionJC1060P470C::displayEndFullFramePreview() {
  g_camera_presenter.end();
}

void DeviceGuitionJC1060P470C::displayWaitDMA() {
}

void DeviceGuitionJC1060P470C::displayCommit() {
...
void DeviceGuitionJC1060P470C::displayFillScreen(uint16_t color) {
  if (!g_panel) {
    return;
  }
...
void DeviceGuitionJC1060P470C::displaySetRotation(uint8_t rotation) {
  g_rotation = rotation & 0x03;
}

void DeviceGuitionJC1060P470C::setBrightness(uint8_t value) {
  g_brightness = value;
  apply_backlight(value);
}

uint8_t DeviceGuitionJC1060P470C::getBrightness() {
  return g_brightness;
}

bool DeviceGuitionJC1060P470C::getTouch(int16_t& x, int16_t& y) {
  if (!g_touch) {
    return false;
  }
...
void DeviceGuitionJC1060P470C::displaySleep() {
  if (g_panel) {
...
void DeviceGuitionJC1060P470C::displayWake() {
  if (g_panel) {
...
void DeviceGuitionJC1060P470C::displayWakeDark() {
  if (g_panel) {
    esp_lcd_panel_disp_on_off(g_panel, true);
  }
  apply_backlight(0);
...
void DeviceGuitionJC1060P470C::displayPowerSaveOn() {
  displaySleep();
}

void DeviceGuitionJC1060P470C::displayPowerSaveOff() {
  displayWake();
}

void DeviceGuitionJC1060P470C::displayWaitDisplay() {
}

void DeviceGuitionJC1060P470C::prepareForRestart() {
  if (g_panel) {
    displayFillScreen(0x0000);
    esp_lcd_panel_disp_on_off(g_panel, false);
  }
...
bool DeviceGuitionJC1060P470C::initSDCard() {
...
bool DeviceGuitionJC1060P470C::storageReady() {
  return g_littlefs_ready;
}

fs::FS& DeviceGuitionJC1060P470C::storageFS() {
  return LittleFS;
}

bool DeviceGuitionJC1060P470C::sdReady() {
  return initSDCard();
}
...
bool DeviceGuitionJC1060P470C::suspendSDCardForNetworkTransition() {
...
bool DeviceGuitionJC1060P470C::resumeSDCardAfterNetworkTransition() {
  g_sd_available = false;
  g_sd_init_attempted = false;
  const bool mounted = initSDCard();
  Serial.printf("[Device/Guition JC1060P470C] SD card remount after network transition: %s\n",
                mounted ? "OK" : "failed");
  return mounted;
}

bool DeviceGuitionJC1060P470C::initLittleFS() {
  if (g_littlefs_ready) {
    return true;
  }
  if (!LittleFS.begin(true, "/littlefs", 10, "spiffs")) {
    Serial.println("[Device/Guition JC1060P470C] LittleFS mount failed");
    return false;
  }
  g_littlefs_ready = true;
  Serial.printf("[Device/Guition JC1060P470C] LittleFS ready, total=%u, used=%u\n",
...
void DeviceGuitionJC1060P470C::migrateStorageFromSD() {
  if (!initLittleFS()) {
    return;
  }
