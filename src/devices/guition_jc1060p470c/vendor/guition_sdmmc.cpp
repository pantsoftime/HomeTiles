...
void reset_sd_card_power() {
  ...
  Serial.printf("[Device/Guition JC1060P470C] SD power reset: gpio=%d off_ms=%lu on_level=%s\n",
...
bool JC1060SDMMCFS::begin(const char* mountpoint,
  ...
  last_error_phase_ = "none";
  last_error_ = ESP_OK;
  Serial.printf("[Device/Guition JC1060P470C] SD mounted, requested=%d kHz, real=%d kHz, host_flags=0x%lx\n",
                sdmmc_frequency,
                frequency_khz_,
...
void JC1060SDMMCFS::end() {
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC1060P470C] SD unmount failed: %s (0x%x)\n",
                  esp_err_to_name(err),
                  static_cast<unsigned>(err));
  }
  card_ = nullptr;
...
bool JC1060SDMMCFS::testWritable() {
  ...
  if (!card_ || !_impl->mountpoint()) {
    Serial.println("[Device/Guition JC1060P470C] SD write test skipped: card is not mounted");
    return false;
  }
  ...
  auto log_failure = [&](const char* phase, int error_number) {
    ...
    Serial.printf("[Device/Guition JC1060P470C] SD write test failed: phase=%s freq=%d kHz errno=%d (%s) card_status=%s (0x%x)\n",
                  phase,
                  frequency_khz_,
                  error_number,
  ...
  writable_ = true;
  Serial.printf("[Device/Guition JC1060P470C] SD write test OK, freq=%d kHz, bytes=%u\n",
                frequency_khz_,
