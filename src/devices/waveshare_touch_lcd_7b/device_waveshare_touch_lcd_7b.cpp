...
#include "src/devices/waveshare_touch_lcd_7b/vendor/displays_config.h"
#include "src/devices/waveshare_touch_lcd_7b/vendor/ek79007/esp_lcd_ek79007.h"
#include "src/devices/waveshare_touch_lcd_7b/vendor/gt911.h"
#include "src/devices/waveshare_touch_lcd_7b/sdmmc.h"
#include "src/devices/waveshare_touch_lcd_7b/vendor/i2c.h"
...
// a media tile is capped to 3x3 cells, so its widest possible title is 3*156 + 2*16
// = 500 px (see MEDIA_TILE_MAX_SPAN). The popup interaction bands that DO want the
// PPA are wider — forecast/energy charts are 752 px, popup cards 792 px, full-screen
// repaints 1024 px. So the gate sits between them: above the 500 px capped title
...
uint8_t g_rotation = DeviceWaveshareTouchLCD7B::kProfile.rotation_default;
...
void log_step(const char* message) {
  Serial.print("[Device/Waveshare Touch LCD 7B] ");
...
void reset_ppa_client() {
  ...
  if (old) {
    ...
    if (unregister_err != ESP_OK) {
      ...
      Serial.printf("[Device/Waveshare Touch LCD 7B] PPA client busy err=%d, retry reset in %lu ms\n",
  ...
  if (reg_err != ESP_OK || !fresh) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.printf("[Device/Waveshare Touch LCD 7B] PPA reset failed err=%d (int free=%u KB, largest=%u KB), retry in %lu ms\n",
  ...
  if (!g_ppa_async_ready) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.println("[Device/Waveshare Touch LCD 7B] PPA timeout callback unavailable, retrying client reset");
    return;
  }
  g_ppa_reinit_at_ms = 0;
  g_ppa_reset_pending = false;
  Serial.println("[Device/Waveshare Touch LCD 7B] PPA client reset after fault");
}
...
void wait_transfer_done(size_t pixels) {
  ...
  if (xSemaphoreTake(g_transfer_done, timeout) != pdTRUE) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] draw transfer timeout (%u px)\n",
...
void wait_refresh_done() {
  ...
  if (xSemaphoreTake(g_refresh_done, pdMS_TO_TICKS(50)) != pdTRUE) {
    Serial.println("[Device/Waveshare Touch LCD 7B] refresh timeout");
  }
}
...
bool ensure_rotate_buffer(size_t pixels) {
  ...
  if (!g_rotate_buf) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] Rotate buffer allocation failed (%u bytes)\n",
...
bool draw_physical(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] Reject out-of-range draw x=%ld y=%ld w=%ld h=%ld\n",
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] draw_bitmap failed: %d\n", static_cast<int>(err));
    return false;
  }
...
bool init_panel_framebuffer() {
  ...
  const esp_err_t err = esp_lcd_dpi_panel_get_frame_buffer(g_panel,
                                                           ...
                                                           &fb0);
  if (err != ESP_OK || !fb0) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] Panel framebuffer unavailable err=%d fb=%p\n",
  ...
  reset_dirty_rect();
  Serial.printf("[Device/Waveshare Touch LCD 7B] Panel framebuffer OK fb=%p\n", fb0);
  return true;
}
...
bool write_physical_to_panel(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  const int32_t panel_h = display_cfg.height;
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] Reject out-of-range framebuffer draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
...
bool draw_landscape_area(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > logical_w || (y + h) > logical_h) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] Reject out-of-range landscape draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
      ...
      if (g_ppa_async_ready && g_ppa_done) {
        ...
        if (err == ESP_OK) {
          if (xSemaphoreTake(g_ppa_done, pdMS_TO_TICKS(kPpaRotateTimeoutMs)) == pdTRUE) {
            ...
            Serial.printf("[Device/Waveshare Touch LCD 7B] PPA rotate timeout x=%ld y=%ld w=%ld h=%ld -> CPU cooldown\n",
                          static_cast<long>(x), static_cast<long>(y),
                          static_cast<long>(w), static_cast<long>(h));
            note_ppa_fault();
          }
        } else {
          Serial.printf("[Device/Waveshare Touch LCD 7B] PPA rotate submit failed err=%d -> CPU cooldown\n",
                        static_cast<int>(err));
          note_ppa_fault();
        }
      }
...
bool init_backlight() {
  ...
  if (ledc_timer_config(&timer_cfg) != ESP_OK) {
    Serial.println("[Device/Waveshare Touch LCD 7B] Backlight timer init failed");
    return false;
  }
  ...
  if (ledc_channel_config(&ch_cfg) != ESP_OK) {
    Serial.println("[Device/Waveshare Touch LCD 7B] Backlight channel init failed");
    return false;
  }
...
bool init_i2c() {
  ...
  g_i2c = DEV_I2C_Init();
  if (!g_i2c.bus) {
    Serial.println("[Device/Waveshare Touch LCD 7B] I2C init failed");
    return false;
  }
  g_i2c_ready = true;
  return true;
...
bool init_mipi_phy_power() {
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] MIPI PHY LDO init failed err=%d\n",
                  static_cast<int>(err));
    return false;
  }
...
bool init_display() {
  ...
  if (!g_transfer_done || !g_refresh_done) {
    Serial.println("[Device/Waveshare Touch LCD 7B] Transfer semaphore allocation failed");
    return false;
  }
  ...
  Serial.printf("[Device/Waveshare Touch LCD 7B] DSI bus init start lane=%u Mbps\n",
                static_cast<unsigned>(display_cfg.lane_bit_rate));
  Serial.flush();
  esp_err_t err = esp_lcd_new_dsi_bus(&bus_cfg, &g_dsi_bus);
  if (err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] DSI bus init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] DSI DBI IO init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  log_step("EK79007 panel create start");
  err = esp_lcd_new_panel_ek79007(g_panel_io, &panel_cfg, &g_panel);
  if (err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] EK79007 panel create failed err=%d\n", static_cast<int>(err));
    return false;
  }
  log_step("EK79007 panel create OK");
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] DPI callback register failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] Panel reset failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] Panel init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] Panel display on failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (ppa_err != ESP_OK) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] PPA client register failed err=%d, falling back to CPU rotate\n",
    ...
    if (g_ppa_done) {
      ...
      if (ppa_client_register_event_callbacks(g_ppa_handle, &ppa_cbs) == ESP_OK) {
        ...
        Serial.println("[Device/Waveshare Touch LCD 7B] PPA event cb register failed, retrying safely");
      }
    }
    if (!g_ppa_async_ready) {
      g_ppa_reset_pending = true;
...
bool init_touch() {
  ...
  for (uint8_t attempt = 1; attempt <= 3; ++attempt) {
    ...
    Serial.printf("[Device/Waveshare Touch LCD 7B] Touch init attempt %u failed\n",
                  static_cast<unsigned>(attempt));
    delay(150);
  }

  if (!g_touch) {
    Serial.println("[Device/Waveshare Touch LCD 7B] Touch init failed");
    return false;
  }
...
bool DeviceWaveshareTouchLCD7B::init() {
  Serial.println("[Device/Waveshare Touch LCD 7B] Initialising board...");
  ...
  Serial.println("[Device/Waveshare Touch LCD 7B] Display OK");

  if (!init_touch()) {
    Serial.println("[Device/Waveshare Touch LCD 7B] Touch init failed, continuing without touch");
  } else {
    Serial.println("[Device/Waveshare Touch LCD 7B] Touch OK");
  }
  ...
  displayFillScreen(0x0000);
  apply_backlight(0);
  Serial.println("[Device/Waveshare Touch LCD 7B] Init complete");
  return true;
}

void DeviceWaveshareTouchLCD7B::update() {
}

void DeviceWaveshareTouchLCD7B::pausePpaFor(uint32_t duration_ms) {
  pause_ppa_for(duration_ms);
}

bool DeviceWaveshareTouchLCD7B::ppaCooldownActive() {
  return ppa_cooldown_active();
}

void DeviceWaveshareTouchLCD7B::displayPushPixels(int32_t x, int32_t y, int32_t w, int32_t h,
                                                 const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}

void DeviceWaveshareTouchLCD7B::displayPushPixelsDMA(int32_t x, int32_t y, int32_t w, int32_t h,
                                                    const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}

bool DeviceWaveshareTouchLCD7B::displayTryFullFramePreview(
    int32_t x, int32_t y, int32_t w, int32_t h,
    int32_t source_stride, const uint16_t* data, size_t data_size,
    bool byte_swap) {
  // Keep this path independent from the normal flush so LVGL remains the
  // fallback for every failure. Never CPU-rotate the roughly 2 MB full image.
  ...
  // Do not wait for a parallel JPEG decode. After this short deadline the
  // established LVGL path is faster and remains the safe fallback.
...
void DeviceWaveshareTouchLCD7B::displayCommit() {
...
void DeviceWaveshareTouchLCD7B::displayFillScreen(uint16_t color) {
  if (!g_panel) {
    return;
  }
...
void DeviceWaveshareTouchLCD7B::displaySetRotation(uint8_t rotation) {
  g_rotation = rotation & 0x03;
}

void DeviceWaveshareTouchLCD7B::setBrightness(uint8_t value) {
  g_brightness = value;
  apply_backlight(value);
}

uint8_t DeviceWaveshareTouchLCD7B::getBrightness() {
  return g_brightness;
}

bool DeviceWaveshareTouchLCD7B::getTouch(int16_t& x, int16_t& y) {
  if (!g_touch) {
    return false;
  }
  ...
  if (!g_touch_active) {
    ...
    if (adx <= kTouchJitterThresholdPx && ady <= kTouchJitterThresholdPx) {
      ...
      // GT911 already applies its own controller-side debounce/filtering.
...
void DeviceWaveshareTouchLCD7B::displayPowerSaveOn() {
  displaySleep();
}

void DeviceWaveshareTouchLCD7B::displayPowerSaveOff() {
  displayWake();
}

void DeviceWaveshareTouchLCD7B::displayWaitDisplay() {
}

void DeviceWaveshareTouchLCD7B::prepareForRestart() {
  if (g_panel) {
    displayFillScreen(0x0000);
    esp_lcd_panel_disp_on_off(g_panel, false);
  }
...
bool DeviceWaveshareTouchLCD7B::initSDCard() {
  if (g_sd_available && Waveshare7BSDMMC.cardType() != CARD_NONE) {
    return true;
  }
  ...
  Waveshare7BSDMMC.end();

  if (!Waveshare7BSDMMC.begin("/sdcard", false, SDMMC_FREQ_HIGHSPEED, 5)) {
    g_sd_available = false;
    Serial.println("[Device/Waveshare Touch LCD 7B] SD card mount failed at 40 MHz");
    return false;
  }

  const sdcard_type_t card_type = Waveshare7BSDMMC.cardType();
  if (card_type == CARD_NONE) {
    g_sd_available = false;
    Serial.println("[Device/Waveshare Touch LCD 7B] SD card absent after mount");
    Waveshare7BSDMMC.end();
    return false;
  }

  g_sd_available = true;
  Serial.printf("[Device/Waveshare Touch LCD 7B] SD card OK, type=%u, size=%llu MB, freq=40000 kHz\n",
                static_cast<unsigned>(card_type),
                static_cast<unsigned long long>(Waveshare7BSDMMC.cardSize() /
                                                (1024ULL * 1024ULL)));
  return true;
}

bool DeviceWaveshareTouchLCD7B::storageReady() {
  return g_littlefs_ready;
}

fs::FS& DeviceWaveshareTouchLCD7B::storageFS() {
  return LittleFS;
}

bool DeviceWaveshareTouchLCD7B::sdReady() {
  return initSDCard();
}

fs::FS& DeviceWaveshareTouchLCD7B::sdFS() {
  return Waveshare7BSDMMC;
}

bool DeviceWaveshareTouchLCD7B::suspendSDCardForNetworkTransition() {
  const bool was_mounted =
      g_sd_available && Waveshare7BSDMMC.cardType() != CARD_NONE;
  if (!was_mounted) return false;

  Waveshare7BSDMMC.end();
  g_sd_available = false;
  g_sd_init_attempted = false;
  Serial.println("[Device/Waveshare Touch LCD 7B] SD card suspended for network transition");
  return true;
}

bool DeviceWaveshareTouchLCD7B::resumeSDCardAfterNetworkTransition() {
  g_sd_available = false;
  g_sd_init_attempted = false;
  const bool mounted = initSDCard();
  Serial.printf("[Device/Waveshare Touch LCD 7B] SD card remount after network transition: %s\n",
                mounted ? "OK" : "failed");
  return mounted;
}

bool DeviceWaveshareTouchLCD7B::initLittleFS() {
  if (g_littlefs_ready) {
    return true;
  }
  if (!LittleFS.begin(true, "/littlefs", 10, "spiffs")) {
    Serial.println("[Device/Waveshare Touch LCD 7B] LittleFS mount failed");
    return false;
  }
  g_littlefs_ready = true;
  Serial.printf("[Device/Waveshare Touch LCD 7B] LittleFS ready, total=%u, used=%u\n",
...
void DeviceWaveshareTouchLCD7B::migrateStorageFromSD() {
  if (!initLittleFS()) {
    return;
  }
  ...
  if (initSDCard()) {
    ...
    copyDirectory(Waveshare7BSDMMC, LittleFS, "/_tile_grids");
    copyDirectory(Waveshare7BSDMMC, LittleFS, "/_tile_links");
    copyDirectory(Waveshare7BSDMMC, LittleFS, "/icons");
