...
bool init_panel_framebuffer() {
  ...
  if (err != ESP_OK || !fb0 || !fb1) {
    Serial.printf("[Device/Waveshare Touch LCD 7B] Panel framebuffers unavailable err=%d fb0=%p fb1=%p\n",
  ...
  if (!g_panel_fb_ready) {
    Serial.println(
        "[Device/Waveshare Touch LCD 7B] Shared camera presenter init failed");
    return false;
  }
  reset_dirty_rect();
  Serial.printf("[Device/Waveshare Touch LCD 7B] Panel framebuffers OK fb0=%p fb1=%p\n",
                fb0, fb1);
  return true;
}
    ...
    if (fb) {
        ...
        if (err == ESP_OK) {
          if (xSemaphoreTake(g_ppa_done,
            ...
            p4_dsi_camera_presenter::restartAfterPpaTimeout(
                "WaveshareTouchLCD7B", "LVGL PPA rotation", x, y, w, h, w,
      ...
      if (ppa_ok) {
        if (!g_camera_presenter.noteUiWrite(
          ...
          Serial.println(
              "[Device/Waveshare Touch LCD 7B] UI framebuffer cache sync failed");
          return false;
        }
...
void DeviceWaveshareTouchLCD7B::displayEndFullFramePreview() {
...
void DeviceWaveshareTouchLCD7B::displaySleep() {
  // The EK79007 panel driver has no display-on/off operation. Waveshare's
  // board support package controls sleep and wake through the backlight.
  apply_backlight(0);
}
