...
#include "src/devices/waveshare_touch_lcd_4_3/vendor/gt911.h"
#include "src/devices/waveshare_touch_lcd_4_3/vendor/i2c.h"
...
// A narrower partial flush can stall it and hold the engine's single pending slot
// ("ppa_srm: exceed maximum pending transactions").
// The shared recovery path heals that condition, but it still costs a cooldown.
// Keep narrow tile updates on the CPU while reserving the PPA for wide popup and
// full-screen bands. The 800x480 profile's widest 3-cell tile is below 500 px,
// while its popup card and full-screen updates are wider than this threshold.
...
uint8_t g_rotation = DeviceWaveshareTouchLCD4_3::kProfile.rotation_default;
...
void log_step(const char* message) {
  Serial.print("[Device/WaveshareTouchLCD4_3] ");
...
void reset_ppa_client() {
  ...
  if (old) {
    ...
    if (unregister_err != ESP_OK) {
      ...
      Serial.printf("[Device/WaveshareTouchLCD4_3] PPA client busy err=%d, retry reset in %lu ms\n",
  ...
  if (reg_err != ESP_OK || !fresh) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.printf("[Device/WaveshareTouchLCD4_3] PPA reset failed err=%d (int free=%u KB, largest=%u KB), retry in %lu ms\n",
  ...
  if (!g_ppa_async_ready) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.println("[Device/WaveshareTouchLCD4_3] PPA timeout callback unavailable, retrying client reset");
    return;
  }
  g_ppa_reinit_at_ms = 0;
  g_ppa_reset_pending = false;
  Serial.println("[Device/WaveshareTouchLCD4_3] PPA client reset after fault");
}
...
void wait_transfer_done(size_t pixels) {
  ...
  if (xSemaphoreTake(g_transfer_done, timeout) != pdTRUE) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] draw transfer timeout (%u px)\n",
...
void wait_refresh_done() {
  ...
  if (xSemaphoreTake(g_refresh_done, pdMS_TO_TICKS(50)) != pdTRUE) {
    Serial.println("[Device/WaveshareTouchLCD4_3] refresh timeout");
  }
}
...
bool ensure_rotate_buffer(size_t pixels) {
  ...
  if (!g_rotate_buf) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] Rotate buffer allocation failed (%u bytes)\n",
...
bool draw_physical(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] Reject out-of-range draw x=%ld y=%ld w=%ld h=%ld\n",
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] draw_bitmap failed: %d\n", static_cast<int>(err));
    return false;
  }
...
bool init_panel_framebuffer() {
  ...
  if (err != ESP_OK || !fb0 || !fb1) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] Panel framebuffers unavailable err=%d fb0=%p fb1=%p\n",
  ...
  if (!g_panel_fb_ready) {
    Serial.println(
        "[Device/WaveshareTouchLCD4_3] Shared camera presenter init failed");
    return false;
  }
  reset_dirty_rect();
  Serial.printf("[Device/WaveshareTouchLCD4_3] Panel framebuffers OK fb0=%p fb1=%p\n",
                fb0, fb1);
  return true;
}
...
bool write_physical_to_panel(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  const int32_t panel_h = display_cfg.height;
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] Reject out-of-range framebuffer draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
...
bool draw_landscape_area(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > logical_w || (y + h) > logical_h) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] Reject out-of-range landscape draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
    ...
    if (fb) {
        ...
        if (err == ESP_OK) {
          if (xSemaphoreTake(g_ppa_done,
            ...
            p4_dsi_camera_presenter::restartAfterPpaTimeout(
                "WaveshareTouchLCD4_3", "LVGL PPA rotation", x, y, w, h, w,
                g_rotation, kPpaRotateTimeoutMs + kPpaPreviewGraceMs);
          }
        } else {
          Serial.printf("[Device/WaveshareTouchLCD4_3] PPA rotate submit failed err=%d -> CPU cooldown\n",
                        static_cast<int>(err));
          note_ppa_fault();
        }
      }
      ...
      if (ppa_ok) {
        if (!g_camera_presenter.noteUiWrite(
          ...
          Serial.println(
              "[Device/WaveshareTouchLCD4_3] UI framebuffer cache sync failed");
          return false;
        }
...
bool init_backlight() {
  ...
  if (ledc_timer_config(&timer_cfg) != ESP_OK) {
    Serial.println("[Device/WaveshareTouchLCD4_3] Backlight timer init failed");
    return false;
  }
  ...
  if (ledc_channel_config(&ch_cfg) != ESP_OK) {
    Serial.println("[Device/WaveshareTouchLCD4_3] Backlight channel init failed");
    return false;
  }
...
bool init_i2c() {
  ...
  g_i2c = DEV_I2C_Init();
  if (!g_i2c.bus) {
    Serial.println("[Device/WaveshareTouchLCD4_3] I2C init failed");
    return false;
  }
  g_i2c_ready = true;
  return true;
...
bool init_mipi_phy_power() {
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] MIPI PHY LDO init failed err=%d\n",
                  static_cast<int>(err));
    return false;
  }
...
bool init_display() {
  ...
  if (!g_transfer_done || !g_refresh_done) {
    Serial.println("[Device/WaveshareTouchLCD4_3] Transfer semaphore allocation failed");
    return false;
  }
  ...
  Serial.printf("[Device/WaveshareTouchLCD4_3] DSI bus init start lane=%u Mbps\n",
                static_cast<unsigned>(display_cfg.lane_bit_rate));
  Serial.flush();
  esp_err_t err = esp_lcd_new_dsi_bus(&bus_cfg, &g_dsi_bus);
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] DSI bus init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] DSI DBI IO init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  vendor_cfg.init_cmds = kWaveshareSt7701Init4_3;
  vendor_cfg.init_cmds_size =
      static_cast<uint16_t>(kWaveshareSt7701Init4_3Count);
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] Panel create failed err=%d\n",
                  static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] DPI callback register failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] Panel reset failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] Panel init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] Panel display on failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (ppa_err != ESP_OK) {
    Serial.printf("[Device/WaveshareTouchLCD4_3] PPA client register failed err=%d, falling back to CPU rotate\n",
    ...
    if (g_ppa_done) {
      ...
      if (ppa_client_register_event_callbacks(g_ppa_handle, &ppa_cbs) == ESP_OK) {
        ...
        Serial.println("[Device/WaveshareTouchLCD4_3] PPA event cb register failed, retrying safely");
      }
    }
    if (!g_ppa_async_ready) {
      g_ppa_reset_pending = true;
...
bool init_touch() {
  ...
  for (uint8_t attempt = 1; attempt <= 3; ++attempt) {
    ...
    Serial.printf("[Device/WaveshareTouchLCD4_3] Touch init attempt %u failed\n",
                  static_cast<unsigned>(attempt));
    delay(150);
  }

  if (!g_touch) {
    Serial.println("[Device/WaveshareTouchLCD4_3] Touch init failed");
    return false;
  }
...
bool DeviceWaveshareTouchLCD4_3::init() {
  Serial.println("[Device/WaveshareTouchLCD4_3] Initialising board...");
  ...
  Serial.println("[Device/WaveshareTouchLCD4_3] Display OK");

  if (!init_touch()) {
    Serial.println("[Device/WaveshareTouchLCD4_3] Touch init failed, continuing without touch");
  } else {
    Serial.println("[Device/WaveshareTouchLCD4_3] Touch OK");
  }
  ...
  displayFillScreen(0x0000);
  apply_backlight(0);
  Serial.println("[Device/WaveshareTouchLCD4_3] Init complete");
  return true;
}

void DeviceWaveshareTouchLCD4_3::update() {
}

void DeviceWaveshareTouchLCD4_3::pausePpaFor(uint32_t duration_ms) {
  pause_ppa_for(duration_ms);
}

bool DeviceWaveshareTouchLCD4_3::ppaCooldownActive() {
  return ppa_cooldown_active();
}

void DeviceWaveshareTouchLCD4_3::displayPushPixels(int32_t x, int32_t y, int32_t w, int32_t h,
                                                 const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}

void DeviceWaveshareTouchLCD4_3::displayPushPixelsDMA(int32_t x, int32_t y, int32_t w, int32_t h,
                                                    const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}

bool DeviceWaveshareTouchLCD4_3::displayTryFullFramePreview(
...
void DeviceWaveshareTouchLCD4_3::displayEndFullFramePreview() {
  g_camera_presenter.end();
}

void DeviceWaveshareTouchLCD4_3::displayWaitDMA() {
}

void DeviceWaveshareTouchLCD4_3::displayCommit() {
...
void DeviceWaveshareTouchLCD4_3::displayFillScreen(uint16_t color) {
  if (!g_panel) {
    return;
  }
...
void DeviceWaveshareTouchLCD4_3::displaySetRotation(uint8_t rotation) {
  g_rotation = rotation & 0x03;
}

void DeviceWaveshareTouchLCD4_3::setBrightness(uint8_t value) {
  g_brightness = value;
  apply_backlight(value);
}

uint8_t DeviceWaveshareTouchLCD4_3::getBrightness() {
  return g_brightness;
}

bool DeviceWaveshareTouchLCD4_3::getTouch(int16_t& x, int16_t& y) {
  if (!g_touch) {
    return false;
  }
...
void DeviceWaveshareTouchLCD4_3::displaySleep() {
  if (g_panel) {
...
void DeviceWaveshareTouchLCD4_3::displayWake() {
  if (g_panel) {
...
void DeviceWaveshareTouchLCD4_3::displayWakeDark() {
  if (g_panel) {
    esp_lcd_panel_disp_on_off(g_panel, true);
  }
  apply_backlight(0);
...
void DeviceWaveshareTouchLCD4_3::displayPowerSaveOn() {
  displaySleep();
}

void DeviceWaveshareTouchLCD4_3::displayPowerSaveOff() {
  displayWake();
}

void DeviceWaveshareTouchLCD4_3::displayWaitDisplay() {
}

void DeviceWaveshareTouchLCD4_3::prepareForRestart() {
  if (g_panel) {
    displayFillScreen(0x0000);
    esp_lcd_panel_disp_on_off(g_panel, false);
  }
...
bool DeviceWaveshareTouchLCD4_3::initSDCard() {
  ...
  if (!WaveshareSDMMC.begin("/sdcard", false, SDMMC_FREQ_DEFAULT, 5)) {
    ...
    Serial.println("[Device/WaveshareTouchLCD4_3] SD card mount failed");
    return false;
  }
  ...
  if (card_type == CARD_NONE) {
    g_sd_available = false;
    Serial.println("[Device/WaveshareTouchLCD4_3] SD card absent after mount");
    WaveshareSDMMC.end();
    return false;
  }

  g_sd_available = true;
  Serial.printf("[Device/WaveshareTouchLCD4_3] SD card OK, type=%u, size=%llu MB\n",
                static_cast<unsigned>(card_type),
                static_cast<unsigned long long>(WaveshareSDMMC.cardSize() / (1024ULL * 1024ULL)));
  return true;
}

bool DeviceWaveshareTouchLCD4_3::storageReady() {
  return g_littlefs_ready;
}

fs::FS& DeviceWaveshareTouchLCD4_3::storageFS() {
  return LittleFS;
}

bool DeviceWaveshareTouchLCD4_3::sdReady() {
  return initSDCard();
}

fs::FS& DeviceWaveshareTouchLCD4_3::sdFS() {
  return WaveshareSDMMC;
}

bool DeviceWaveshareTouchLCD4_3::suspendSDCardForNetworkTransition() {
  ...
  Serial.println("[Device/WaveshareTouchLCD4_3] SD card suspended for network transition");
  return true;
}

bool DeviceWaveshareTouchLCD4_3::resumeSDCardAfterNetworkTransition() {
  g_sd_available = false;
  g_sd_init_attempted = false;
  const bool mounted = initSDCard();
  Serial.printf("[Device/WaveshareTouchLCD4_3] SD card remount after network transition: %s\n",
                mounted ? "OK" : "failed");
  return mounted;
}

bool DeviceWaveshareTouchLCD4_3::initLittleFS() {
  if (g_littlefs_ready) {
    return true;
  }
  if (!LittleFS.begin(true, "/littlefs", 10, "spiffs")) {
    Serial.println("[Device/WaveshareTouchLCD4_3] LittleFS mount failed");
    return false;
  }
  g_littlefs_ready = true;
  Serial.printf("[Device/WaveshareTouchLCD4_3] LittleFS ready, total=%u, used=%u\n",
...
void DeviceWaveshareTouchLCD4_3::migrateStorageFromSD() {
  if (!initLittleFS()) {
    return;
  }
