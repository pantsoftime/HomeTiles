...
#include "src/devices/layout_test_480x480/vendor/i2c.h"
#include "src/devices/layout_test_480x480/vendor/jd9365/esp_lcd_jd9365.h"
...
uint8_t g_rotation = DeviceLayoutTest480x480::kProfile.rotation_default;
...
void log_step(const char* message) {
  Serial.print("[Device/LayoutTest480x480] ");
...
void reset_ppa_client() {
  ...
  if (old) {
    ...
    if (unregister_err != ESP_OK) {
      ...
      Serial.printf("[Device/LayoutTest480x480] PPA client busy err=%d, retry reset in %lu ms\n",
  ...
  if (reg_err != ESP_OK || !fresh) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.printf("[Device/LayoutTest480x480] PPA reset failed err=%d (int free=%u KB, largest=%u KB), retry in %lu ms\n",
  ...
  if (!g_ppa_async_ready) {
    g_ppa_reinit_at_ms = millis() + kPpaReinitRetryMs;
    if (!g_ppa_reinit_at_ms) g_ppa_reinit_at_ms = 1;
    Serial.println("[Device/LayoutTest480x480] PPA timeout callback unavailable, retrying client reset");
    return;
  }
  g_ppa_reinit_at_ms = 0;
  g_ppa_reset_pending = false;
  Serial.println("[Device/LayoutTest480x480] PPA client reset after fault");
}
...
void wait_transfer_done(size_t pixels) {
  ...
  if (xSemaphoreTake(g_transfer_done, timeout) != pdTRUE) {
    Serial.printf("[Device/LayoutTest480x480] draw transfer timeout (%u px)\n",
...
void wait_refresh_done() {
  ...
  if (xSemaphoreTake(g_refresh_done, pdMS_TO_TICKS(50)) != pdTRUE) {
    Serial.println("[Device/LayoutTest480x480] refresh timeout");
  }
}
...
bool ensure_rotate_buffer(size_t pixels) {
  ...
  if (!g_rotate_buf) {
    Serial.printf("[Device/LayoutTest480x480] Rotate buffer allocation failed (%u bytes)\n",
...
bool draw_physical(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/LayoutTest480x480] Reject out-of-range draw x=%ld y=%ld w=%ld h=%ld\n",
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] draw_bitmap failed: %d\n", static_cast<int>(err));
    return false;
  }
...
bool init_panel_framebuffer() {
  ...
  const esp_err_t err = esp_lcd_dpi_panel_get_frame_buffer(g_panel,
                                                           ...
                                                           &fb0);
  if (err != ESP_OK || !fb0) {
    Serial.printf("[Device/LayoutTest480x480] Panel framebuffer unavailable err=%d fb=%p\n",
  ...
  reset_dirty_rect();
  Serial.printf("[Device/LayoutTest480x480] Panel framebuffer OK fb=%p\n", fb0);
  return true;
}
...
bool write_physical_to_panel(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  const int32_t panel_h = display_cfg.height;
  if (x < 0 || y < 0 || (x + w) > panel_w || (y + h) > panel_h) {
    Serial.printf("[Device/LayoutTest480x480] Reject out-of-range framebuffer draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
...
bool draw_landscape_area(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t* data) {
  ...
  if (x < 0 || y < 0 || (x + w) > logical_w || (y + h) > logical_h) {
    Serial.printf("[Device/LayoutTest480x480] Reject out-of-range landscape draw x=%ld y=%ld w=%ld h=%ld\n",
                  static_cast<long>(x), static_cast<long>(y),
                  static_cast<long>(w), static_cast<long>(h));
    return false;
  }
      ...
      if (g_ppa_async_ready && g_ppa_done) {
        ...
        if (err == ESP_OK) {
          if (xSemaphoreTake(g_ppa_done, pdMS_TO_TICKS(kPpaRotateTimeoutMs)) == pdTRUE) {
            ...
            Serial.printf("[Device/LayoutTest480x480] PPA rotate timeout x=%ld y=%ld w=%ld h=%ld -> CPU cooldown\n",
                          static_cast<long>(x), static_cast<long>(y),
                          static_cast<long>(w), static_cast<long>(h));
            note_ppa_fault();
          }
        } else {
          Serial.printf("[Device/LayoutTest480x480] PPA rotate submit failed err=%d -> CPU cooldown\n",
                        static_cast<int>(err));
          note_ppa_fault();
        }
      }
...
bool init_backlight() {
  ...
  if (ledc_timer_config(&timer_cfg) != ESP_OK) {
    Serial.println("[Device/LayoutTest480x480] Backlight timer init failed");
    return false;
  }
  ...
  if (ledc_channel_config(&ch_cfg) != ESP_OK) {
    Serial.println("[Device/LayoutTest480x480] Backlight channel init failed");
    return false;
  }
...
bool init_i2c() {
  ...
  g_i2c = DEV_I2C_Init();
  if (!g_i2c.bus) {
    Serial.println("[Device/LayoutTest480x480] I2C init failed");
    return false;
  }
  g_i2c_ready = true;
  return true;
...
bool init_mipi_phy_power() {
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] MIPI PHY LDO init failed err=%d\n",
                  static_cast<int>(err));
    return false;
  }
...
bool init_display() {
  ...
  if (!g_transfer_done || !g_refresh_done) {
    Serial.println("[Device/LayoutTest480x480] Transfer semaphore allocation failed");
    return false;
  }
  ...
  Serial.printf("[Device/LayoutTest480x480] DSI bus init start lane=%u Mbps\n",
                static_cast<unsigned>(display_cfg.lane_bit_rate));
  Serial.flush();
  esp_err_t err = esp_lcd_new_dsi_bus(&bus_cfg, &g_dsi_bus);
  if (err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] DSI bus init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] DSI DBI IO init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] JD9365 panel create failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] DPI callback register failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] Panel reset failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] Panel init failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] Panel display on failed err=%d\n", static_cast<int>(err));
    return false;
  }
  ...
  if (ppa_err != ESP_OK) {
    Serial.printf("[Device/LayoutTest480x480] PPA client register failed err=%d, falling back to CPU rotate\n",
    ...
    if (g_ppa_done) {
      ...
      if (ppa_client_register_event_callbacks(g_ppa_handle, &ppa_cbs) == ESP_OK) {
        ...
        Serial.println("[Device/LayoutTest480x480] PPA event cb register failed, retrying safely");
      }
    }
    if (!g_ppa_async_ready) {
      g_ppa_reset_pending = true;
...
bool init_touch() {
  ...
  for (uint8_t attempt = 1; attempt <= 3; ++attempt) {
    ...
    Serial.printf("[Device/LayoutTest480x480] Touch init attempt %u failed\n",
                  static_cast<unsigned>(attempt));
    delay(150);
  }

  if (!g_touch) {
    Serial.println("[Device/LayoutTest480x480] Touch init failed");
    return false;
  }
...
bool DeviceLayoutTest480x480::init() {
  Serial.println("[Device/LayoutTest480x480] Initialising board...");
  ...
  Serial.println("[Device/LayoutTest480x480] Display OK");

  if (!init_touch()) {
    Serial.println("[Device/LayoutTest480x480] Touch init failed, continuing without touch");
  } else {
    Serial.println("[Device/LayoutTest480x480] Touch OK");
  }
  ...
  displayFillScreen(0x0000);
  apply_backlight(0);
  Serial.println("[Device/LayoutTest480x480] Init complete");
  return true;
}

void DeviceLayoutTest480x480::update() {
}

void DeviceLayoutTest480x480::pausePpaFor(uint32_t duration_ms) {
  pause_ppa_for(duration_ms);
}

bool DeviceLayoutTest480x480::ppaCooldownActive() {
  return ppa_cooldown_active();
}

void DeviceLayoutTest480x480::displayPushPixels(int32_t x, int32_t y, int32_t w, int32_t h,
                                                 const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}

void DeviceLayoutTest480x480::displayPushPixelsDMA(int32_t x, int32_t y, int32_t w, int32_t h,
                                                    const uint16_t* data) {
  draw_landscape_area(x, y, w, h, data);
}

bool DeviceLayoutTest480x480::displayTryFullFramePreview(
...
void DeviceLayoutTest480x480::displayWaitDMA() {
}

void DeviceLayoutTest480x480::displayCommit() {
...
void DeviceLayoutTest480x480::displayFillScreen(uint16_t color) {
  if (!g_panel) {
    return;
  }
...
void DeviceLayoutTest480x480::displaySetRotation(uint8_t rotation) {
  g_rotation = rotation & 0x03;
}

void DeviceLayoutTest480x480::setBrightness(uint8_t value) {
  g_brightness = value;
  apply_backlight(value);
}

uint8_t DeviceLayoutTest480x480::getBrightness() {
  return g_brightness;
}

bool DeviceLayoutTest480x480::getTouch(int16_t& x, int16_t& y) {
  if (!g_touch) {
    return false;
  }
  ...
  // intentionally exposes only the top-left 480x480 region to LVGL.
...
void DeviceLayoutTest480x480::displaySleep() {
  if (g_panel) {
...
void DeviceLayoutTest480x480::displayWake() {
  if (g_panel) {
...
void DeviceLayoutTest480x480::displayWakeDark() {
  if (g_panel) {
    esp_lcd_panel_disp_on_off(g_panel, true);
  }
  apply_backlight(0);
...
void DeviceLayoutTest480x480::displayPowerSaveOn() {
  displaySleep();
}

void DeviceLayoutTest480x480::displayPowerSaveOff() {
  displayWake();
}

void DeviceLayoutTest480x480::displayWaitDisplay() {
}

void DeviceLayoutTest480x480::prepareForRestart() {
  if (g_panel) {
    displayFillScreen(0x0000);
    esp_lcd_panel_disp_on_off(g_panel, false);
  }
...
bool DeviceLayoutTest480x480::initSDCard() {
  ...
  if (!WaveshareSDMMC.begin("/sdcard", false, SDMMC_FREQ_DEFAULT, 5)) {
    ...
    Serial.println("[Device/LayoutTest480x480] SD card mount failed");
    return false;
  }
  ...
  if (card_type == CARD_NONE) {
    g_sd_available = false;
    Serial.println("[Device/LayoutTest480x480] SD card absent after mount");
    WaveshareSDMMC.end();
    return false;
  }

  g_sd_available = true;
  Serial.printf("[Device/LayoutTest480x480] SD card OK, type=%u, size=%llu MB\n",
                static_cast<unsigned>(card_type),
                static_cast<unsigned long long>(WaveshareSDMMC.cardSize() / (1024ULL * 1024ULL)));
  return true;
}

bool DeviceLayoutTest480x480::storageReady() {
  return g_littlefs_ready;
}

fs::FS& DeviceLayoutTest480x480::storageFS() {
  return LittleFS;
}

bool DeviceLayoutTest480x480::sdReady() {
  return initSDCard();
}

fs::FS& DeviceLayoutTest480x480::sdFS() {
  return WaveshareSDMMC;
}

bool DeviceLayoutTest480x480::suspendSDCardForNetworkTransition() {
  ...
  Serial.println("[Device/LayoutTest480x480] SD card suspended for network transition");
  return true;
}

bool DeviceLayoutTest480x480::resumeSDCardAfterNetworkTransition() {
  g_sd_available = false;
  g_sd_init_attempted = false;
  const bool mounted = initSDCard();
  Serial.printf("[Device/LayoutTest480x480] SD card remount after network transition: %s\n",
                mounted ? "OK" : "failed");
  return mounted;
}

bool DeviceLayoutTest480x480::initLittleFS() {
  if (g_littlefs_ready) {
    return true;
  }
  if (!LittleFS.begin(true, "/littlefs", 10, "spiffs")) {
    Serial.println("[Device/LayoutTest480x480] LittleFS mount failed");
    return false;
  }
  g_littlefs_ready = true;
  Serial.printf("[Device/LayoutTest480x480] LittleFS ready, total=%u, used=%u\n",
...
void DeviceLayoutTest480x480::migrateStorageFromSD() {
  if (!initLittleFS()) {
    return;
  }
...
#endif  // defined(DEVICE_LAYOUT_TEST_480X480)
