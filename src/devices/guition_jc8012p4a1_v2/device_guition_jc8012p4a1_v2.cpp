...
bool init_panel_framebuffer() {
  ...
  const esp_err_t err = esp_lcd_dpi_panel_get_frame_buffer(g_panel,
                                                           ...
                                                           &fb0);
  if (err != ESP_OK || !fb0) {
    Serial.printf("[Device/Guition JC8012P4A1 V2] Panel framebuffer unavailable err=%d fb=%p\n",
  ...
  reset_dirty_rect();
  Serial.printf("[Device/Guition JC8012P4A1 V2] Panel framebuffer OK fb=%p\n", fb0);
  return true;
}
