...
esp_lcd_touch_handle_t touch_gt911_init(DEV_I2C_Port port)
{
    ...
    // Select the documented 0x5D address deterministically. GT911 samples INT
    // while reset is released; driving INT low avoids depending on board pulls.
...
static esp_err_t touch_gt911_reset(esp_lcd_touch_handle_t tp)
{
    ...
    if (tp->config.rst_gpio_num != GPIO_NUM_NC) {
        if (tp->config.int_gpio_num != GPIO_NUM_NC) {
            ESP_RETURN_ON_ERROR(gpio_set_direction(tp->config.int_gpio_num, GPIO_MODE_OUTPUT),
                                TAG, "INT GPIO direction error");
            ESP_RETURN_ON_ERROR(gpio_set_level(tp->config.int_gpio_num, 0),
                                TAG, "INT GPIO set level error");
        }
        ...
        if (tp->config.int_gpio_num != GPIO_NUM_NC) {
            ESP_RETURN_ON_ERROR(gpio_set_direction(tp->config.int_gpio_num, GPIO_MODE_INPUT),
                                TAG, "INT GPIO direction error");
        }
        vTaskDelay(pdMS_TO_TICKS(50));
    }
...
static esp_err_t touch_gt911_read_cfg(esp_lcd_touch_handle_t tp)
{
    ...
    Serial.printf("[Device/Guition JC1060P470C V2] TouchPad_ID:%02X%02X%02X cfg=%u\n",
