...
// Remember the last logged dump fingerprint so normal reboots do not
// append the same dump repeatedly.
...
// Identify a dump across reboots using PC, TCB, RA and its size.
...
void logBootDiagnostics() {
  ...
  // Normal boot without a new dump: nothing to log. A known fingerprint
  // prevents appending an already recorded dump again.
  bool log_dump = false;
  String fingerprint;
  if (dump_valid) {
